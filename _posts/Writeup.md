...
| telnet     | guest (empty password) / anonymous:anonymous |

> **Note**: The root password is the first 15 characters of a SHA-256 hash generated by the `photonctl` MIPS binary. It is deterministic based on the firmware version files and arthur.dev's password as token.
...
- `qemu-user-static` (specifically `qemu-mipsel`) is installed to allow execution of the MIPS binary on the x86 host. This is intentional and part of the exploitation path.
...
## Discovery — Developer Notes and History
...
```bash
arthur.dev@mipsy:~$ file ~/firmware-test/squashfs-root/usr/bin/photonctl
```
...
```
[+] Token validated.
[+] Root credential: <15_HEX_CHARS>
```

The tool validates our token and outputs a "Root credential" — a 15-character hex string derived from firmware metadata and the developer token.
